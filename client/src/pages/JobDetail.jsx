...
import { CustomButton, JobCard, Loading, ApplicationForm } from "../components";
...
const JobDetail = () => {
  ...
  return (
    <div className="container mx-auto mt-10">
      <div className="w-fit flex flex-col md:flex-row gap-10">
        {/* LEFT SIDE */}
        {isFetching ? (
          <Loading />
        ) : (
            ...
            <div className="w-full flex gap-4 py-5">
              <CustomButton
                ...
                containerStyles={`w-full flex items-center justify-center py-3 px-5 outline-none rounded-full text-sm ${
                  selected === "0"
                    ? "bg-gradient-to-r from-[#ab43ed] to-[#e44d62] text-white font-semibold"
              ...
              {user?.accountType === "company" ? (
                  ...
                  {user?._id === job?.company?._id ? (
                    <div className="w-full flex gap-6">
                      <CustomButton
                        ...
                        containerStyles={`w-full flex items-center justify-center text-white bg-blue-600 py-3 px-5 outline-none rounded-full text-md hover:text-blue-600 hover:bg-blue-400 font-semibold`}
                      />
                      <CustomButton
                        title="Delete Post"
                        onClick={handleDeletePost}
                        containerStyles={`w-full flex items-center justify-center text-white bg-red-600 py-3 px-5 outline-none rounded-full text-md hover:bg-red-400 hover:text-red-900 font-semibold`}
                      />
                    </div>
                  ) : (
                    <CustomButton
                      title="Login as seeker to apply for this job"
                      containerStyles={`w-full flex items-center justify-center text-white bg-gradient-to-r from-[#ab43ed] to-[#e44d62] py-3 px-5 outline-none rounded-full text-base cursor-pointer font-semibold`}
                    />
                  )}
                </>
              ) : (
                <>
                  {user?.appliedJobs.includes(job?._id) ? (
                    <Link to="/apply-history">
                      <CustomButton
                        title="You have already applied for this job"
                        containerStyles={`w-full flex items-center justify-center text-white bg-gradient-to-r from-[#ab43ed] to-[#e44d62] py-3 px-5 outline-none rounded-full text-base cursor-pointer`}
                      />
                    </Link>
                  ) : (
                    <div className="container mx-auto mt-10">
                      {showApplicationForm ? (
                        ...
                        <CustomButton
                          ...
                          title="Apply Now"
                          containerStyles={`w-full flex items-center justify-center text-white bg-gradient-to-r from-[#ab43ed] to-[#e44d62] py-3 px-5 outline-none rounded-full text-base`}
                        />
                      )}
                    </div>
                  )}
